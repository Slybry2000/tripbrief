  ...
  return <dialog ref={ref} className="reply-reader" aria-label={item ? `Reply from ${item.operator}` : "Reply"} onClose={() => onIndex(null)} onKeyDown={(event) => { if (event.key === "ArrowRight") step(1); if (event.key === "ArrowLeft") step(-1); }} onClick={(event) => { if (event.target === event.currentTarget) onIndex(null); }}>
    {item && <div className="reader-shell">
      <header className="reader-head">
        ...
        <div className="reader-nav">{items.length > 1 && <><button className="secondary" onClick={() => step(-1)} aria-label="Previous reply">← Previous</button><button className="secondary" onClick={() => step(1)} aria-label="Next reply">Next →</button></>}<button className="secondary" onClick={() => onIndex(null)} aria-label="Close">Close</button></div>
      </header>
...
function ReplyImport({ briefId, shortlist, request }: { briefId: Id<"briefs">; shortlist: ShortlistRow[]; request: TripRequest }) {
  ...
  return <section className="workflow-section reply-panel"><div className="section-heading"><p className="eyebrow">EMAILED REPLIES</p><h2>An operator can answer in its own words</h2><p>A reply that arrives in the workspace mail inbox is matched to the operator it came from, and to the request its thread belongs to. Paste it here and a model drafts the structured proposal, quoting the reply for every claim — then you correct it and record it. Nothing is recorded until you press the button.</p></div>
    ...
    {replies.length > 0 && <div className="inbox-list">{replies.map((message, position) => { const answered = shortlist.find((row) => row.operatorSlug === message.operatorSlug)?.status === "submitted"; return <article key={message._id}><div><small>{new Date(message.receivedAt).toLocaleString()} · {message.fromName || message.fromEmail}</small><h3>{message.subject || "(no subject)"}</h3><p>{message.text.slice(0, 240)}{message.text.length > 240 ? "…" : ""}</p></div><div className="brief-actions"><button className="secondary" onClick={() => setReading(position)}>Read full reply</button>{answered ? <span className="status-badge">In the comparison</span> : <button className="primary" disabled={busy} onClick={() => void draft(message.operatorSlug ?? waiting[0]?.operatorSlug ?? "", message.text)}>Draft a proposal from this →</button>}</div></article>; })}</div>}
    <ReplyReader index={reading} onIndex={setReading} items={replies.map((message) => { const answered = shortlist.find((row) => row.operatorSlug === message.operatorSlug)?.status === "submitted"; return { id: message._id, operator: message.fromName || message.fromEmail, subject: message.subject, receivedAt: message.receivedAt, text: message.text, simulated: /\(Simulated reply/.test(message.text), facts: [], action: answered ? undefined : { label: "Draft a proposal from this", run: () => void draft(message.operatorSlug ?? waiting[0]?.operatorSlug ?? "", message.text) } }; })} />
