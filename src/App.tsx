...
// The operator's whole reply, exactly as written, opened in place. Nothing is
// drafted or recorded by reading it.
function FullReply({ text, label = "Read the full reply" }: { text: string; label?: string }) {
  if (!text.trim()) return null;
  return <details className="full-reply"><summary>{label}</summary><div className="full-reply-text">{text}</div></details>;
}
...
function Workspace() {
  ...
  const viewContent = activeView === "dashboard" ? <Dashboard start={start} portal={() => goView("portal")} briefs={briefList ?? []} open={openBrief} remove={deleteBriefById} demo={Boolean(account?.demo)} />
...
function ReplyImport({ briefId, shortlist, request }: { briefId: Id<"briefs">; shortlist: ShortlistRow[]; request: TripRequest }) {
  ...
  return <section className="workflow-section reply-panel"><div className="section-heading"><p className="eyebrow">EMAILED REPLIES</p><h2>An operator can answer in its own words</h2><p>A reply that arrives in the workspace mail inbox is matched to the operator it came from, and to the request its thread belongs to. Paste it here and a model drafts the structured proposal, quoting the reply for every claim — then you correct it and record it. Nothing is recorded until you press the button.</p></div>
    ...
    {replies.length > 0 && <div className="inbox-list">{replies.map((message) => { const answered = shortlist.find((row) => row.operatorSlug === message.operatorSlug)?.status === "submitted"; return <article key={message._id}><div><small>{new Date(message.receivedAt).toLocaleString()} · {message.fromName || message.fromEmail}</small><h3>{message.subject || "(no subject)"}</h3><p>{message.text.slice(0, 240)}{message.text.length > 240 ? "…" : ""}</p><FullReply text={message.text} /></div>{answered ? <span className="status-badge">In the comparison</span> : <button className="primary" disabled={busy} onClick={() => void draft(message.operatorSlug ?? waiting[0]?.operatorSlug ?? "", message.text)}>Draft a proposal from this →</button>}</article>; })}</div>}
